...
def render_agent_scaling_tab(analysis, config):
    ...
    with col1:
        st.metric(
            ...
            delta=f"{(agent_analysis.get('primary_tool') or 'Unknown').upper()} {(agent_analysis.get('agent_size') or 'Unknown').title()}"
        )
    ...
    if network_bw > actual_throughput:
        ...
        nic_type = config.get('nic_type', 'gigabit_fiber')
        os_name = (config.get('operating_system', 'Unknown') or 'Unknown').replace('_', ' ').title()
        server_type = config.get('server_type', 'physical')
        num_agents = config.get('number_of_agents', 1)
        tool_name = (analysis.get('primary_tool') or 'DMS').upper()
    ...
    with col1:
        ...
        with st.container():
            ...
            st.write(f"**Agent Type:** {(agent_analysis.get('primary_tool') or 'Unknown').upper()}")
            st.write(f"**Agent Size:** {(agent_analysis.get('agent_size') or 'Unknown').title()}")
    ...
    with col2:
        ...
        if optimal_recommendations.get('optimal_configuration'):
            ...
            with st.container():
                ...
                st.write(f"**Optimal Size:** {(optimal_config['configuration']['agent_size'] or 'Unknown').title()}")
